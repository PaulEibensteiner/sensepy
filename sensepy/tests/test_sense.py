...
def test_capture_epsilon_greedy():
    ...
    Bandit = EpsilonGreedySense(
        process, estimator, w, epsilon=lambda x: 0.5, initial_data=data, dt=dt
    )
    T = 5
    for t in range(T):
        Bandit.step(Sets)
...
def test_top2_count_record(example_setup_count_record):
    ...
    estimator.steps = 10
    Bandit = Top2Thompson(
        process, estimator, w, D, n, initial_data=data, dt=dt, level_set=2
    )
    T = 5
    for t in range(T):
        Bandit.step(actions)
