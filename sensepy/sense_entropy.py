...
class SenseEntropy(SensingAlgorithm):
    ...
    def __init__(
        ...
        beta: float = 3,
    ) -> None:  # confidence parameter used for ucb,lcb calculation
        """
    ...
    def get_ucb_lcb(
        ...
        if self.approx is None:
            self.mean, self.lcb, self.ucb = self.estimator.map_lcb_ucb(
                D, n, beta=self.beta
            )
    ...
    def per_action_acquisiton_function_V(self, action: BorelSet) -> float:
        ...
        for i in range(self.repeats):
            ...
            if obs is not None:
                ...
                U = torch.diag(1.0 / np.sqrt(torch.abs(norm.view(-1)) + jitter)) @ Phi
                Sigma = (
                    self.Phi_R.T
                    @ self.Phi_R
                    @ (
                        ...
                        @ U.T
                        @ torch.pinverse(
                            torch.eye(U.size()[0]).double() + U @ self.invSigma @ U.T
                        )
                        @ U
                        @ self.invSigma
                    )
    ...
    def per_action_acquisiton_function_V_approx(self, action):
        ...
        for i in range(self.repeats):
            ...
            if obs is not None:
                ...
                norm = self.estimator.mean_rate_points(obs)
                U = torch.diag(1.0 / np.sqrt(torch.abs(norm.view(-1)) + 10e-5)) @ Phi
    ...
    def acquisition_function(self, actions: List) -> torch.Tensor:
        ...
        if self.design_type != "Random":  # if not random do the below
            ...
            self.new_data = []
            self.newPhi = []
            surogate_process = PoissonPointProcess(
                d=self.process.d, B=self.process.B, b=self.process.b
            )
        ...
        for id_action, action in enumerate(actions):
            if self.design_type == "V":
                ...
                raise NotImplementedError(
                    "The requested acquisiton function is not implemented."
                )
        return scores
