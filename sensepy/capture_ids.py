...
class CaptureIDS(CaptureUCB):
    ...
    def acquisition_function(self, actions: List) -> torch.Tensor:
        ...
        if self.original == True:
            ...
            gaps = [
                self.estimator.gap(action, actions, self.w, dt=self.dt)
                for action in actions
            ]
            inf = [
                self.estimator.information(
                    action, self.dt, precomputed=self.precomputed
                )
                for action in actions
            ]
            gaps = np.array(gaps)
    ...
    def acquisition_function_original(self, actions):
        ...
        ratio = lambda p: (
            gaps_squared[index1] * p + gaps_squared[index2] * (1 - p)
        ) / (inf[index1] * p + inf[index2] * (1 - p))
    ...
    def step(self, actions, verbose: bool = False, points: bool = False):
        """
