...
class BeilschmiediaProblem(SpatialProblem):
    ...
    def load_data(self, clusters=100, prefix="../data/"):
        ...
        # transform to [-1,1]
        self.Xtsub, trans, inv_trans, tr, _ = transform(
            torch.from_numpy(Xsub), offsets=[(0, 1000), (0, 500)]
        )
        self.X2tsub, trans, inv_trans, tr, _ = transform(
            torch.from_numpy(X2sub), offsets=[(0, 1000), (0, 500)]
        )
    ...
    def fit_model(self):
        ...
        self.kernel1 = KernelFunction(
            d=1, kernel_name="squared_exponential", gamma=self.gamma, kappa=1
        )
        self.kernel2 = KernelFunction(
            d=1, kernel_name="squared_exponential", gamma=self.gamma, kappa=1
        )
        kernel = (
            lambda x, y, kappa, group, offset=0: kappa
            * 1000
            * self.kernel1.kernel(self.height(x), self.height(y))
            * self.kernel1.kernel(self.slope(x), self.slope(y))
        )
