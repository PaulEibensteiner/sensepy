...
class CholeraProblem(SpatialProblem):
    ...
    def __init__(self, n=40, m=30, b=0.0, gamma=0.5):
        super().__init__(m)
        self.n = n
        self.m = m
        self.b = b
    ...
    def load_data(self, name_prefix="../data/"):
        ...
        gdf = geopandas.GeoDataFrame(
            ...
            geometry=geopandas.points_from_xy(
                locations_deaths["Y coordinate"], locations_deaths["X coordinate"]
            ),
        )
        ...
        gdf2 = geopandas.GeoDataFrame(
            locations_pumps,
            geometry=geopandas.points_from_xy(
                locations_pumps["Y coordinate"], locations_pumps["X coordinate"]
            ),
        )
        ...
        self.transform_x = lambda x: (2 / (x_max - x_min)) * x + (
            1 - (2 * x_max / (x_max - x_min))
        )
        self.transform_y = lambda y: (2 / (y_max - y_min)) * y + (
        ...
        def k_func(x, y, kappa=1.0, group=None):
